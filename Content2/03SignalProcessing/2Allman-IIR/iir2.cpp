...
float 	IIR_filter(float newsample, RingBuffer &samples, CoefBuffer &coeff);
bool 	InitBuffers(RingBuffer &rb, CoefBuffer &cb);

/*-----------------------------------------
Inputs:
	Series of iir coefficients from file
...
int main(int argc, char *argv[])
{
   ...
   samples.resize(coeff.size()-2); // size sample FIFO to filter order
   //printf("size: %d\n", coeff.size());

   //InitRingBuffer(samples);


   float output	    		= 0.0; //variable to store output of IIR in

   //read samples in from stdin
   while(fgets(string, BUFFERSIZE, stdin)) {
	output = IIR_filter(atof(string), samples, coeff);
	printf("%f\n", output);		
   }


   for(int n = coeff.size()/2; n; n--) { //flush input samples all the way through the filter
	output = IIR_filter(0.0, samples, coeff);
	printf("%f\n", output);
   }
}
...
float IIR_filter(float newsample, RingBuffer &samples, CoefBuffer &coeff)
{
   ...
   float output = newsample*coeff[0];

   //start at the end
   int rindex,sindex=0;
   for(rindex= samples.size()/2-1; rindex >=0; rindex--,sindex++) {
      output += samples[sindex] * coeff[rindex];
      output -= samples[sindex+samples.size()/2] * coeff[rindex+coeff.size()/2];
   }

   //shift the register
   samples.pop_back();
   samples.push_front(newsample);
   samples[samples.size()/2] = output;
