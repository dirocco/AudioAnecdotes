...
#ifndef INCLUDED_STOCHASTICAMPLITUDEESSENCE_H
#define INCLUDED_STOCHASTICAMPLITUDEESSENCE_H

#include "Random.h"

/*
    StochasticAmplitudeEssence models:
    concept AmplitudeEssenceT {
        float amplitude();
    }
*/

class StochasticAmplitudeEssence{
public:
    StochasticAmplitudeEssence() : amplitudeGenerator_( 0., 1.) {}

    StochasticAmplitudeEssence& amplitude( double min, double max )
        { amplitudeGenerator_.range( min, max ); return *this; }

    float amplitude() const { return amplitudeGenerator_.generate(); }
        
private:
    mutable RangedDoubleRandomNumberGenerator amplitudeGenerator_;
};

#endif /* INCLUDED_STOCHASTICAMPLITUDEESSENCE_H */
